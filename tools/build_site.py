...
/* --- testsidan --- */
.progress { background:var(--chip); border-radius:6px; height:9px; margin:.4rem 0 1.6rem; overflow:hidden; }
.progress i { display:block; height:100%; background:var(--go); border-radius:6px; }
@media (prefers-reduced-motion: no-preference) { .progress i { transition:width .25s ease; } }
.qmeta { display:flex; justify-content:space-between; font:500 .74rem/1.4 ui-monospace, monospace;
...
.qtext { font-family:Georgia, serif; font-size:1.35rem; line-height:1.35; margin:.2rem 0 1rem;
  text-wrap:balance; }
details.info { margin:0 0 1.2rem; border:1px solid var(--line); border-radius:8px;
  background:var(--chip); font-size:.9rem; }
details.info summary { cursor:pointer; padding:.65rem .9rem; font-weight:600;
  color:var(--accent-ink); list-style-position:inside; }
details.info[open] summary { border-bottom:1px solid var(--line); }
details.info .info-body { padding:.8rem .95rem 1rem; }
details.info .info-body > p { margin:0 0 .8rem; }
.proscons { display:grid; grid-template-columns:1fr 1fr; gap:1rem; }
@media (max-width:560px) { .proscons { grid-template-columns:1fr; } }
.proscons h4 { margin:0 0 .35rem; font-size:.8rem; text-transform:uppercase; letter-spacing:.06em; }
.proscons .pro h4 { color:var(--pro); } .proscons .con h4 { color:var(--con); }
.proscons ul { margin:0; padding-left:1.1rem; }
.proscons li { margin:.25rem 0; }
.answers { display:grid; gap:.5rem; margin:0 0 1rem; }
.answers button { text-align:left; font-size:1rem; padding:.7rem 1rem; border-radius:8px;
...
.weight { display:flex; align-items:center; gap:.5rem; flex-wrap:wrap; margin-bottom:1.1rem;
  font-size:.85rem; color:var(--ink-2); }
.weight .seg { display:inline-flex; border:1px solid var(--line); border-radius:8px; overflow:hidden; }
.weight .seg button { border:0; background:var(--paper); color:var(--ink-2); padding:.42rem .8rem;
  font-size:.82rem; cursor:pointer; }
.weight .seg button + button { border-left:1px solid var(--line); }
.weight .seg button.sel { background:var(--accent); color:#3a2f00; font-weight:600; }
.qnav { display:flex; justify-content:space-between; align-items:center; gap:1rem; }
.qnav button, .btn { font-size:.95rem; padding:.6rem 1.3rem; border-radius:8px; cursor:pointer;
  border:1.5px solid var(--line); background:var(--card); color:var(--ink); }
.qnav button:disabled { opacity:.4; cursor:default; }
.btn-go { background:var(--go); border-color:var(--go); color:var(--go-ink); font-weight:700; }
.btn-go:hover { background:var(--go-hover); border-color:var(--go-hover); }
.skip { background:none; border:none; color:var(--ink-2); text-decoration:underline;
...
.review-item { border-bottom:1px solid var(--line); padding:.9rem 0; }
.review-item .rq { font-weight:600; }
.review-item .ra { font-size:.9rem; margin-top:.2rem; }
...
<section id="v-quiz" class="hidden">
  ...
  <div class="qcard">
    ...
    <details class="info" id="q-info">
      <summary>ℹ️&nbsp; Vad handlar frågan om? För- och nackdelar</summary>
      <div class="info-body">
        <p id="q-expl"></p>
        <div class="proscons">
...
<section id="v-review" class="hidden">
  <h1>Dina svar</h1>
  <p class="small">Sparade lokalt i din webbläsare. Öppna infoknappen för att läsa om frågan igen.</p>
  <div id="rev-list"></div>
  <div class="startrow">
    <button class="btn" id="rev-back" type="button">← Till resultatet</button>
    <a class="btn" href="index.html" style="text-decoration:none">Till startsidan</a>
  </div>
</section>
...
function renderStart() {{
  ...
  if (touched === 0) {{
    ...
    row.appendChild(btn('Börja om', 'btn', restart));
    box.appendChild(row);
  }}
}}
...
function restart() {{
  ...
  answers = {{}}; idx = 0; show('v-quiz'); renderQ();
}}
...
function renderQ() {{
  ...
  el('q-next').className = (q.id in answers) ? 'btn-go' : '';
...
el('r-redo').addEventListener('click', restart);
...
function renderReview() {{
  ...
  QUESTIONS.forEach(function (q, i) {{
    ...
    var ansTxt = a ? labelFor(a.v) : 'Ej besvarad ännu';
    var wTxt = a && a.v !== null && a.v !== undefined ? ' · vikt: ' + weightFor(a.w || 1) : '';
    item.innerHTML = '<div class="rq">' + (i + 1) + '. ' + q.text + '</div>' +
      '<div class="ra">Ditt svar: <b>' + ansTxt + '</b>' + wTxt + '</div>';
    var det = document.createElement('details'); det.className = 'info';
    det.innerHTML = '<summary>ℹ️&nbsp; Om frågan</summary>';
